...
var last_built_node : Node3D = null
...
func recalculate_grid_stress(grid: Array[Node3D]) -> void:
	var power: float = 0.0
	for node : Node3D in grid:
...
func recalculate_all_grids() -> void:
	...
	for node: Node3D in  GlobalScript.build_list.all_power_nodes:
		if not visited.has(node) and node:
			var grid: Array[Node3D] = find_whole_grid_bfs(node)
			on_network_change(node, grid)
			visited.append_array(grid)

func on_network_change(start_node: Node3D, grid:Array[Node3D] = []) -> void:
	if grid == []:
	...
	var broken_node_connections: Array[Node3D] = solve_speeds(generators)
...
func solve_speeds(generators: Array[Generator]) -> Array[Node3D]:
	...
	while not queue.is_empty():
		var current_node : Node3D= queue.pop_front()
		if current_node in visited: continue
		visited.append(current_node)
		current_node.speed = node_speeds[current_node]
		for local_port: Area3D in current_node.connections:
			for connection in current_node.connections[local_port]:
				...
				if node_speeds.has(connection.node) and not is_equal_approx(proposed_connection_speed, node_speeds[connection.node]):
					if is_zero_approx(proposed_connection_speed):
						...
						if last_built_node:
							var broken_node_connections: Array[Node3D] = last_built_node.get_connections()
...
func find_whole_grid_bfs(start_node: Node3D) -> Array[Node3D]:
	var visited: Array[Node3D] = []
...
func break_priority(node1 : Node3D, node2 : Node3D) -> void:
	# First case:
