...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("openBuildingMenu"):
		if buildingMenu.is_visible_in_tree():
			...
			GlobalScript.opened_gui = buildingMenu
			cancel_placement()
			buildingMenu.show()
...
func place() -> void:
	...
	canPlace = false
	var current_data: Resource = get_data_from_slot(selected_index)
	instance = null
	if current_data:
...
func select_hotbar_slot(index: int) -> void:
	...
	if isPlacing and instance:
		instance.queue_free()
		instance = null
	var data: Resource = get_data_from_slot(index)
	if data:
		instantiate_building(data)
	else:
		isPlacing = false

func get_data_from_slot(index: int) -> Resource:
	if index < hotbar.get_child_count():
		var slot: PanelContainer = hotbar.get_child(index)
